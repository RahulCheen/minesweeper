...
### Core Logic
*   **Guaranteed Opening**: Mine generation is deferred until the user's first click. The algorithm explicitly excludes the clicked tile and its 8 immediate surrounding neighbors from mine deployment, guaranteeing a clear 3x3 initial opening.
...
*   **Dynamic Scaling**: The Pygame window correctly traps `VIDEORESIZE` events. The internal rendering surface calculates optimal cell square dimensions bounded dynamically by desktop bounds and the currently requested grid columns and rows.
*   **State Tracking**: 
    *   **Timer**: Starts exactly on the first click. Displays fractional decimal precision on game resolution (win/loss).
